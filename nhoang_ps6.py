...
outVars = ['subj', 'sess', 'scene', 'trial', 'stimOn', 'questOn', 'response', 'rt', 'correctNum', '%accuracy']    
...
incText = visual.TextStim(win, text='incorrect')
...
fbClock = core.Clock() # will reset after question presentation

corNum = 0 # record number of correct trials
...
for thisTrial in np.arange(0,nTrials):
    ...
    # read the correct response to this trial
    context = trialInfo.loc[thisTrial,'context']
    ...
    # give feedback
    if trialResp == 'i' and context == 'indoor':
        corText.draw()
        corNum = corNum + 1
    if trialResp == 'i' and context == 'outdoor':
        incText.draw()
        corNum = corNum + 0
    if trialResp == 'o' and context == 'outdoor':
        corText.draw()
        corNum = corNum + 1
    elif trialResp == 'o' and context == 'indoor':
        incText.draw()
        corNum = corNum + 0
    win.flip()
    fbClock.reset() # for fb presentation duration
    ...
    out.loc[thisTrial, 'correctNum'] = corNum
    out.loc[thisTrial, '%accuracy'] = (corNum/(thisTrial + 1))*100
